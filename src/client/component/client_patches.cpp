...
namespace client_patches
{
	namespace
	{
		...
		extern "C" void migrate_if_needed()
		{
			...
			if (std::filesystem::is_directory("t7x/players", e))
			{
				return;
			}

			utils::io::create_directory("t7x/players");

			std::filesystem::copy("players", "t7x/players", std::filesystem::copy_options::recursive, e);
		}
